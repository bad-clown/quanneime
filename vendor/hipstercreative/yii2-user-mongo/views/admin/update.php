...
<h1><i class="glyphicon glyphicon-user"></i> <?= Html::encode($model->username) ?>
    ...
    <?php if ($model->getIsBlocked()): ?>
        <?= Html::a(Yii::t('user', 'Unblock'), ['block', 'id' => (string)$model->_id], ['class' => 'btn btn-success btn-xs', 'data-method' => 'post', 'data-confirm' => Yii::t('user', 'Are you sure to block this user?')]) ?>
    <?php else: ?>
        <?= Html::a(Yii::t('user', 'Block'), ['block', 'id' => (string)$model->_id], ['class' => 'btn btn-danger btn-xs', 'data-method' => 'post', 'data-confirm' => Yii::t('user', 'Are you sure to block this user?')]) ?>
    <?php endif; ?>
</h1>
...
<div class="panel panel-default">
    ...
    <div class="panel-body">
        ...
        <?= $form->field($model, 'definedVipLevel')->dropDownList(ArrayHelper::map(Dictionary::indexList("VIP"),"key","key")) ?>
